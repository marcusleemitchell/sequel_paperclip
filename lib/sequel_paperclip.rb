...
require 'tempfile'
require File.join(File.dirname(__FILE__), 'sequel_paperclip', 'upfile')
require File.join(File.dirname(__FILE__), 'sequel_paperclip', 'iostream')
require File.join(File.dirname(__FILE__), 'sequel_paperclip', 'geometry')
require File.join(File.dirname(__FILE__), 'sequel_paperclip', 'thumbnail')
require File.join(File.dirname(__FILE__), 'sequel_paperclip', 'storage')
require File.join(File.dirname(__FILE__), 'sequel_paperclip', 'attachment')
...
module Paperclip
  ...
  module Resource
    def self.included(base)
      ...
      # Register merb root interpolation
      Paperclip::Attachment.interpolations[:merb_root] = lambda{|attachment,style| Merb.root }
    end
  end
  ...
  module ClassMethods
    ...
    # Sequel doesn't have column_names
    def column_names
  ...
  module InstanceMethods #:nodoc:
    ...
    def save_attached_files
      ...
      begin 
        each_attachment do |name, attachment|
          raise PaperclipError unless attachment.send(:save)
        end
    ...
    # Sequel doesn't have attribute_get and paperclip needs that
    def attribute_get(name)
      self[name]
    end

    # Sequel uses set() instead of attributes
